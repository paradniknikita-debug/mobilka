...
class ObjectPropertiesPanel extends ConsumerWidget {
  ...
  static String _conditionLabel(dynamic value) {
    final raw = (value?.toString() ?? '').trim().toLowerCase();
    if (raw.isEmpty) return 'не указано';
    switch (raw) {
      case 'good':
        return 'Хорошее';
      case 'satisfactory':
  ...
  static String _normalizePoleTitle(dynamic value) {
    ...
    if (raw.toLowerCase().startsWith(prefix.toLowerCase())) {
      ...
      return cut.isEmpty ? 'не указано' : cut;
    }
    return raw;
  }
  ...
  String get _title {
    switch (objectType) {
      ...
      case ObjectType.substation:
        return 'Подстанция ${objectProperties['name']?.toString() ?? 'не указано'}';
      case ObjectType.tap:
        return 'Отпайка ${objectProperties['tap_number']?.toString() ?? 'не указано'}';
      case ObjectType.equipment:
        return 'Оборудование ${objectProperties['name']?.toString() ?? 'не указано'}';
    }
  }
  ...
  List<Widget> _buildPoleProperties(BuildContext context) {
    ...
    return [
      _buildPropertyItem(
        context,
        'Название:',
        objectProperties['pole_number']?.toString() ?? objectProperties['poleNumber']?.toString() ?? 'не указано',
      ),
      _buildPropertyItem(
        context,
        'UID (MRID):',
      ...
      _buildPropertyItem(
        context,
        'Состояние:',
        _conditionLabel(objectProperties['condition']),
      ),
      _buildPropertyItem(
        context,
        'Примечания:',
        objectProperties['notes']?.toString().trim().isNotEmpty == true
            ? objectProperties['notes'].toString()
            : 'не указано',
        isLast: seq == null,
      ),
      if (seq != null)
  ...
  List<Widget> _buildSubstationProperties(BuildContext context) {
    return [
      _buildPropertyItem(
        ...
        objectProperties['dispatcher_name']?.toString() ?? 'не указано',
      ),
      _buildPropertyItem(
        context,
        'Напряжение:',
        '${objectProperties['voltage_level']?.toString() ?? 'не указано'} кВ',
      ),
      _buildPropertyItem(
        context,
        'Адрес:',
        objectProperties['address']?.toString() ?? 'не указано',
      ),
      _buildPropertyItem(
        context,
        _coordsLabel,
  ...
  List<Widget> _buildTapProperties(BuildContext context) {
    return [
      _buildPropertyItem(
        context,
        'Тип:',
        objectProperties['tap_type']?.toString() ?? 'не указано',
      ),
      _buildPropertyItem(
        context,
        'Напряжение:',
  ...
  List<Widget> _buildEquipmentProperties(BuildContext context) {
    ...
    return [
      ...
      _buildPropertyItem(
        context,
        'Тип:',
        objectProperties['equipment_type']?.toString() ?? 'не указано',
      ),
      _buildPropertyItem(
        context,
        'Опора:',
        objectProperties['pole_number']?.toString() ??
            objectProperties['pole_id']?.toString() ??
            'не указано',
      ),
      _buildPropertyItem(
        context,
  ...
  Widget _buildPropertyItem(
    ...
    final canCopy =
        copyable && normalizedValue.isNotEmpty && normalizedValue.toLowerCase() != 'не указано';
    return Container(
