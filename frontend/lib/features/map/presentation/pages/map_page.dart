...
class _MapPageState extends ConsumerState<MapPage> {
  ...
  /// Радиус погрешности GPS (м), с платформы — для круга на карте.
  double? _locationAccuracyMeters;
  /// Направление движения (°), если платформа отдаёт валидный heading.
  double? _locationHeadingDegrees;
  ...
  /// Левая часть номера отпайки N/M: порядковый номер отпайки в линии (1, 2, 3…), не номер опоры-якоря.
  String? _currentTapRoot;
  /// Индекс ветки от опоры-якоря (1, 2, …) для текущей отпайки.
  int? _currentTapBranchIndex;
  /// ID отпаечной опоры-якоря для FAB и диалога; обязателен вместе с [_currentTapRoot].
  int? _currentTapAnchorPoleId;
  ...
  void _applyGpsPosition(Position position, {required bool centerMapIfReady}) {
    final latLng = LatLng(position.latitude, position.longitude);
    final h = position.heading;
    final headingOk = h.isFinite && h >= 0 && h <= 360;
    if (mounted) {
      setState(() {
        _currentLocation = latLng;
        _locationAccuracyMeters = position.accuracy.isFinite && position.accuracy > 0 ? position.accuracy : null;
        _locationHeadingDegrees = headingOk ? h : null;
      });
    }
    if (centerMapIfReady && _mapReady) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && _currentLocation != null) {
          try {
            _mapController.move(_currentLocation!, AppConfig.defaultZoom);
          } catch (e) {
            if (kDebugMode) print('Ошибка центрирования карты: $e');
          }
        }
      });
    } else if (centerMapIfReady && !_mapReady) {
      _pendingCenterOnLocation = latLng;
    }
  }

  DateTime? _lastNavigatorMapFollowAt;

  /// Непрерывные обновления координат на карте (пока экран открыт).
  /// В режиме навигатора дополнительно с троттлингом центрируем камеру.
  void _startLocationTracking() {
    ...
    _positionSubscription = Geolocator.getPositionStream(
      ...
      (position) {
        _applyGpsPosition(position, centerMapIfReady: false);
        if (!mounted || !_isNavigatorMode) return;
        final now = DateTime.now();
        if (_lastNavigatorMapFollowAt != null &&
            now.difference(_lastNavigatorMapFollowAt!).inMilliseconds < 1500) {
          return;
        }
        _lastNavigatorMapFollowAt = now;
        final loc = LatLng(position.latitude, position.longitude);
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted || !_isNavigatorMode) return;
          try {
            final z = _mapController.camera.zoom;
            final targetZ = z < 18 ? 18.0 : z;
            _mapController.move(loc, targetZ);
          } catch (_) {}
        });
      },
      onError: (_) {},
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      // На веб getLastKnownPosition не реализован и бросает — из-за этого раньше
      // весь try падал в catch (Минск), без getCurrentPosition и без стрима.
      if (!kIsWeb) {
        final lastKnown = await Geolocator.getLastKnownPosition();
        if (lastKnown != null) {
          _applyGpsPosition(lastKnown, centerMapIfReady: false);
        }
      }
      ...
      _applyGpsPosition(position, centerMapIfReady: true);
      _startLocationTracking();
    } catch (e) {
      if (kDebugMode) print('Ошибка получения местоположения: $e');
      if (mounted) setState(() {
        _currentLocation = const LatLng(53.9045, 27.5615);
      });
    }
      ...
      body: _isLoading
          ...
          : Stack(
              children: [
                FlutterMap(
                  ...
                  children: [
                    ...
                    if (_currentLocation != null &&
                        ...
                        _locationAccuracyMeters! > 2)
                      CircleLayer(
                        circles: [
                          CircleMarker(
                            point: _currentLocation!,
                            radius: _locationAccuracyMeters!.clamp(8.0, 400.0),
                      ...
                      MarkerLayer(
                        markers: [
                          Marker(
                            ...
                            child: _locationHeadingDegrees != null
                                ? Transform.rotate(
                                    angle: _locationHeadingDegrees! * math.pi / 180,
                                    child: const Icon(
                                      Icons.navigation,
                ...
                if (_currentLocation != null && _locationAccuracyMeters != null)
                  Positioned(
                    bottom: 88,
                    left: 16,
                    child: Material(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 6),
                            Text(
                              '±${_locationAccuracyMeters!.round()} м',
                                    ...
                                    child: Padding(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            _currentTapBranchIndex != null
                                                ? 'Отпайка $_currentTapRoot/$_currentTapBranchIndex • К магистрали'
  ...
  /// Ключ визуальной ветки опоры.
  /// Номер вида «N/M/…»: первый сегмент N — **номер отпайки** (какая по счёту ветка),
  /// а не номер магистральной опоры; одна и та же «отпайка 1» может идти от опоры 40 или 50.
  /// «Отпайка от отпайки» — номер с **тремя и более** сегментами (например `1/2/1`): ключ по
  /// префиксу без последнего сегмента, чтобы не пересекаться с обычной веткой `1/2`.
  String _poleVisualBranchKey(int lineId, Map<String, dynamic>? props) {
    final poleNumber = (props?['pole_number'] ?? '').toString().trim();
    final segments = poleNumber.split('/').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
    final hasSlash = segments.length >= 2;
    final tapOrdinal = segments.isNotEmpty ? segments.first : '';
    final tapPoleIdFromProps = _toInt(props?['tap_pole_id']);
    final tapBranchIndex = _toInt(props?['tap_branch_index']);
    final nestedPathPrefix =
        segments.length >= 3 ? segments.sublist(0, segments.length - 1).join('/') : null;

    if (hasSlash) {
      if (tapPoleIdFromProps != null) {
        if (nestedPathPrefix != null) {
          if (tapBranchIndex != null) {
            return '$lineId:tap:$tapPoleIdFromProps:sub:$nestedPathPrefix:b:$tapBranchIndex';
          }
          return '$lineId:tap:$tapPoleIdFromProps:sub:$nestedPathPrefix';
        }
        if (tapBranchIndex != null) {
          return '$lineId:tap:$tapPoleIdFromProps:b:$tapBranchIndex';
        }
        if (tapOrdinal.isNotEmpty) {
          return '$lineId:tap:$tapPoleIdFromProps:n:$tapOrdinal';
        }
        return '$lineId:tap:$tapPoleIdFromProps:x';
      }
      if (nestedPathPrefix != null) {
        if (tapBranchIndex != null) {
          return '$lineId:tb:sub:$nestedPathPrefix:b:$tapBranchIndex';
        }
        return '$lineId:tb:sub:$nestedPathPrefix';
      }
      if (tapBranchIndex != null) {
        return '$lineId:tb:$tapBranchIndex';
      }
      if (tapOrdinal.isNotEmpty) {
        return '$lineId:ord:$tapOrdinal';
      }
    }

    final tapPoleIdRaw = props?['tap_pole_id'];
    if (tapPoleIdRaw == null) {
      return '$lineId:main';
    }
    final tapPoleId = _toInt(tapPoleIdRaw);
    if (tapPoleId == null) {
      return '$lineId:main';
    }
    if (tapBranchIndex != null) {
      return '$lineId:$tapPoleId:b:$tapBranchIndex';
    }
    return '$lineId:$tapPoleId:fallback';
  }

  bool _isTapBranchKey(String branchKey, int lineId) => branchKey != '$lineId:main';

  /// Линии ЛЭП: та же схема, что `renderPowerLines` в Angular — группировка опор по ключу ветки,
  /// для каждой фичи LineString из GeoJSON: при совпадении ветки — цепочка по опорам (как на вебе),
  /// иначе координаты из геометрии фичи.
  List<Polyline> _buildPowerLinePolylines() {
    ...
    final polesById = <int, Map<String, dynamic>>{};
    final polesByBranch = <String, List<Map<String, dynamic>>>{};
    ...
    for (final feature in poleFeatures) {
      try {
        ...
        final poleMap = <String, dynamic>{
          ...
          'tap_branch_index': _toInt(props['tap_branch_index']),
          'sequence_number': _toInt(props['sequence_number']),
          'is_tap_pole': props['is_tap_pole'] == true,
          'is_tap': props['is_tap'] == true,
        };
        polesById[poleId] = poleMap;
        final k = _poleVisualBranchKey(lineId, props);
        polesByBranch.putIfAbsent(k, () => []).add(poleMap);
      } catch (_) {}
    }

    for (final list in polesByBranch.values) {
      list.sort((a, b) {
        final sa = _toInt(a['sequence_number']);
        final sb = _toInt(b['sequence_number']);
        if (sa != null && sb != null && sa != sb) return sa.compareTo(sb);
        final oa = _poleOrderFromNumber((a['pole_number'] as String?) ?? '');
        final ob = _poleOrderFromNumber((b['pole_number'] as String?) ?? '');
        if (oa != ob) return oa.compareTo(ob);
        return ((a['pole_number'] as String?) ?? '').compareTo((b['pole_number'] as String?) ?? '');
      });
    }

    // Если опоры есть, рисуем ЛЭП только по ним.
    // Это стабильнее серверных LineString при частично заполненных branch/tap полях:
    // исключает ложные связи между ветками (например, 2/1 -> 1/1).
    if (polesByBranch.isNotEmpty) {
      return _buildPowerLinePolylinesFromPolesOnlyAngular(polesById, polesByBranch);
    }

    void addPolylineSegment(
      List<LatLng> points, {
      required bool isTap,
      required int? lineId,
      required double vEff,
    }) {
      ...
      polylines.add(Polyline(points: points, strokeWidth: stroke, color: color));
    }

    List<LatLng> _coordsToLatLngs(List<dynamic> coordinates) {
      return coordinates.map((coord) {
        final c = coord as List<dynamic>;
        return LatLng(_toDouble(c[1]), _toDouble(c[0]));
      }).toList();
    }
    ...
    for (final feature in features) {
      try {
        ...
        final lineId = _toInt(props?['id'] ?? props?['line_id'] ?? props?['power_line_id']);
        if (lineId == null) continue;

        var branchKey = _poleVisualBranchKey(lineId, props);
        if (!polesByBranch.containsKey(branchKey) && props?['branch_type'] == 'tap') {
          final tpid = _toInt(props?['tap_pole_id']);
          final tbi = _toInt(props?['tap_branch_index']);
          if (tpid != null && tbi != null) {
            for (final pf in poleFeatures) {
              final pprops = pf['properties'] as Map<String, dynamic>?;
              final g = pf['geometry'] as Map<String, dynamic>?;
              if (pprops == null || g == null || g['type'] != 'Point') continue;
              final lid = _toInt(pprops['line_id'] ?? pprops['power_line_id']);
              if (lid != lineId) continue;
              if (_toInt(pprops['tap_pole_id']) != tpid) continue;
              if (_toInt(pprops['tap_branch_index']) != tbi) continue;
              final k = _poleVisualBranchKey(lineId, pprops);
              if (polesByBranch.containsKey(k)) {
                branchKey = k;
                break;
              }
            }
        ...
        final vEff = vlProp > 0 ? vlProp : (_voltageKvForPowerLine(lineId) ?? 0);

        final branchPoles = polesByBranch[branchKey];
        var drewFromPoles = false;

        if (branchPoles != null && branchPoles.isNotEmpty) {
          final poleList = List<Map<String, dynamic>>.from(branchPoles);
          final tapId = _inferTapPoleIdForBranch(poleList, polesById);
          final useEquipmentSegments = poleList.length >= 2;
          final tapBranch = _isTapBranchKey(branchKey, lineId);

          final pts = <LatLng>[];
          if (tapId != null && tapBranch && !useEquipmentSegments) {
            final basePole = polesById[tapId];
            if (basePole != null) {
              pts.add(LatLng(_toDouble(basePole['lat']), _toDouble(basePole['lng'])));
            }
          }

          if (useEquipmentSegments) {
            // Как Angular: соседние опоры ветки; без полюсов на карте ЛЭП — прямые участки.
            // Якорь → первая опора отпайки: иначе при 1/1+1/2 пропадает участок от отпаечной опоры (см. GeoJSON бэкенда).
            if (tapId != null && tapBranch && poleList.isNotEmpty) {
              final basePole = polesById[tapId];
              if (basePole != null) {
                final first = poleList.first;
                addPolylineSegment(
                  [
                    LatLng(_toDouble(basePole['lat']), _toDouble(basePole['lng'])),
                    LatLng(_toDouble(first['lat']), _toDouble(first['lng'])),
                  ],
                  isTap: true,
                  lineId: lineId,
                  vEff: vEff,
            ...
            for (var i = 0; i < poleList.length - 1; i++) {
              final p1 = poleList[i];
              final p2 = poleList[i + 1];
              addPolylineSegment(
                [
                  LatLng(_toDouble(p1['lat']), _toDouble(p1['lng'])),
                  LatLng(_toDouble(p2['lat']), _toDouble(p2['lng'])),
                ],
                isTap: tapBranch,
                lineId: lineId,
                vEff: vEff,
            ...
            for (final p in poleList) {
              pts.add(LatLng(_toDouble(p['lat']), _toDouble(p['lng'])));
            }
            if (pts.length >= 2) {
              addPolylineSegment(pts, isTap: isTap, lineId: lineId, vEff: vEff);
              drewFromPoles = true;
            }
          }
        }
        ...
        if (!drewFromPoles) {
          final raw = _coordsToLatLngs(coordinates);
          if (raw.length >= 2) {
            addPolylineSegment(raw, isTap: isTap, lineId: lineId, vEff: vEff);
          }
        }
      } catch (e) {
    ...
    if (polylines.isEmpty && poleFeatures.isNotEmpty) {
      polylines.addAll(_buildPowerLinePolylinesFromPolesOnlyAngular(polesById, polesByBranch));
    }

    return polylines;
  }

  /// Нет фич LineString в коллекции ЛЭП — строим только по опорам с теми же ключами веток, что в Angular.
  List<Polyline> _buildPowerLinePolylinesFromPolesOnlyAngular(
    Map<int, Map<String, dynamic>> polesById,
    Map<String, List<Map<String, dynamic>>> polesByBranch,
  ) {
    final out = <Polyline>[];
    for (final entry in polesByBranch.entries) {
      final branchKey = entry.key;
      final poleList = entry.value;
      if (poleList.isEmpty) continue;
      final lineIdStr = branchKey.split(':').first;
      final lineId = int.tryParse(lineIdStr);
      if (lineId == null) continue;
      final vEff = _voltageKvForPowerLine(lineId) ?? 0;
      final tapBranch = _isTapBranchKey(branchKey, lineId);
      final tapId = _inferTapPoleIdForBranch(poleList, polesById);
      final useEquipmentSegments = poleList.length >= 2;

      void addSeg(List<LatLng> pts, {required bool isTap}) {
        if (pts.length < 2) return;
        final isCurrentPatrol = lineId == _currentLineId;
        out.add(
          Polyline(
            points: pts,
            strokeWidth: VoltageLevelColors.strokeWidthForLine(isTap: isTap, isPatrol: isCurrentPatrol),
            color: VoltageLevelColors.colorForVoltageKv(vEff > 0 ? vEff : null),
          ),
        );
      }

      // В ветке отпайки ровно одна опора (например только 1/1): раньше ветка отбрасывалась
      // из‑за length < 2 — не рисовался участок от якорной опоры до первой отпаечной.
      if (poleList.length == 1) {
        if (tapBranch && tapId != null) {
          final basePole = polesById[tapId];
          final only = poleList.first;
          if (basePole != null) {
            addSeg(
              [
                LatLng(_toDouble(basePole['lat']), _toDouble(basePole['lng'])),
                LatLng(_toDouble(only['lat']), _toDouble(only['lng'])),
              ],
              isTap: true,
            );
          }
      ...
      if (tapBranch && tapId != null && useEquipmentSegments) {
        final basePole = polesById[tapId];
        if (basePole != null && poleList.isNotEmpty) {
          final first = poleList.first;
          addSeg(
            [
              LatLng(_toDouble(basePole['lat']), _toDouble(basePole['lng'])),
      ...
      } else if (tapBranch && tapId != null && !useEquipmentSegments) {
        final pts = <LatLng>[];
        final basePole = polesById[tapId];
        if (basePole != null) {
          pts.add(LatLng(_toDouble(basePole['lat']), _toDouble(basePole['lng'])));
        ...
        addSeg(pts, isTap: true);
      } else {
        final pts = poleList.map((p) => LatLng(_toDouble(p['lat']), _toDouble(p['lng']))).toList();
        addSeg(pts, isTap: false);
      }
    }
    return out;
  }
  ...
  /// Якорь для первого сегмента ветки на карте.
  /// Для «отпайки от отпайки» (`1/3/1`) линия должна идти от опоры `1/3`, а не от магистральной
  /// (у API [tap_pole_id] по-прежнему якорь магистрали «3»).
  int? _inferTapPoleIdForBranch(
    List<Map<String, dynamic>> poleList,
    Map<int, Map<String, dynamic>> polesById,
  ) {
    if (poleList.isEmpty) return null;
    final firstPn = (poleList.first['pole_number'] as String?)?.trim() ?? '';
    final segments = firstPn
        .split('/')
        .map((e) => e.trim())
        .where((e) => e.isNotEmpty)
        .toList();
    if (segments.length >= 3) {
      final parentNumber = segments.sublist(0, segments.length - 1).join('/');
      for (final p in polesById.values) {
        if (((p['pole_number'] as String?) ?? '').trim() == parentNumber) {
          return _toInt(p['id']);
        }
      }
    }
    for (final p in poleList) {
      final tid = _toInt(p['tap_pole_id']);
  ...
  String _branchKeyLocalPole(drift_db.Pole p) {
    ...
    final parts = pn.split('/').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
    if (parts.length >= 3) {
      return 'tap:sub:${parts.sublist(0, parts.length - 1).join('/')}';
    }
    final root = parts.isNotEmpty ? parts.first : '';
    return root.isEmpty ? 'main' : 'tap:$root';
  }

  /// Как [getPoleVisualBranchKey] в Angular — для группировки сегментов линейного оборудования по ветке.
  String _branchKeyServerPole(Map<String, dynamic> p) {
    final lineId = _toInt(p['line_id'] ?? p['power_line_id']);
    if (lineId == null) return '0:main';
    return _poleVisualBranchKey(lineId, p);
  }
  ...
  Future<void> _showEditPoleDialog() async {
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) => CreatePoleDialog(
        ...
        initialLineVoltageKv: _voltageKvForPowerLine(powerLineId),
      ),
    );
    if (!mounted) return;
  ...
  /// Максимальный корень N в номерах вида N/M по линии (Drift + GeoJSON карты + API).
  Future<int> _maxTapRootForLine(int lineId, List<drift_db.Pole> linePoles) async {
    int maxTapRoot = 0;
    ...
    for (final f in poleFeatures) {
      try {
        ...
        consider(props['pole_number']?.toString());
      } catch (_) {}
    }
    try {
      final api = ref.read(apiServiceProvider);
      final poles = await api.getPoles(lineId);
  ...
  /// Максимальный суффикс M для номеров «{tapRoot}/M» в линии.
  Future<int> _maxTapSuffixForRoot(
    int lineId,
    String tapRoot,
    List<drift_db.Pole> linePoles,
  ) async {
    final root = tapRoot.trim();
    final prefix = '$root/';
    int maxSuffix = 0;
    void consider(String? raw) {
      final n = (raw ?? '').trim();
  ...
  Future<void> _openCreatePoleFromTapPole(int lineId, int tapPoleId, [String? tapPoleNumber]) async {
    ...
    // Корень номера отпайки (левую часть N/M) — последовательный индекс по линии; учитываем сервер и GeoJSON, не только Drift.
    final maxTapRoot = await _maxTapRootForLine(lineId, linePoles);
    final newTapRoot = '${maxTapRoot + 1}';

    // Контекст для FAB: N/M, ветка и id якоря (не искать якорь по номеру — он больше не совпадает с N).
    setState(() {
      _currentTapRoot = newTapRoot;
      _currentTapBranchIndex = nextBranchIndex;
      _currentTapAnchorPoleId = tapPoleId;
    });
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) => CreatePoleDialog(
        ...
        initialLineVoltageKv: _voltageKvForPowerLine(lineId),
      ),
    );
  ...
  Future<void> _showAddPoleToTapDialog() async {
    ...
    final selectedPoleId = _toInt(_selectedObjectProperties?['id']);
    if (lineId == null || !poleNumber.contains('/')) return;
    final partsEarly = poleNumber.split('/');
    if (partsEarly.length < 2) return;
    final rootEarly = partsEarly[0].trim();
    final suffixEarly = int.tryParse(partsEarly[1].trim());
    if (suffixEarly == null) return;

    // Свойства GeoJSON могут не содержать или устареть tap_* — берём актуальные данные с API.
    int? tapPoleId = _toInt(_selectedObjectProperties?['tap_pole_id']);
    int? tapBranchIndex = _toInt(_selectedObjectProperties?['tap_branch_index']);

    final api = ref.read(apiServiceProvider);
    if (selectedPoleId != null) {
      try {
        final pole = await api.getPole(selectedPoleId);
        if (pole.tapPoleId != null) tapPoleId = pole.tapPoleId;
        if (pole.tapBranchIndex != null) tapBranchIndex = pole.tapBranchIndex;
      } catch (_) {}
    }
    if (tapPoleId == null || tapBranchIndex == null) {
      try {
        ...
        Pole? self;
        for (final p in poles) {
          if (selectedPoleId != null && p.id == selectedPoleId) {
            self = p;
            break;
          }
        }
        if (self != null) {
          if (tapPoleId == null && self.tapPoleId != null) tapPoleId = self.tapPoleId;
          if (tapBranchIndex == null && self.tapBranchIndex != null) {
            tapBranchIndex = self.tapBranchIndex;
          }
        }
        if (tapPoleId == null) {
          for (final p in poles) {
            final n = p.poleNumber.trim();
            if (n.startsWith('$rootEarly/') && p.tapPoleId != null) {
              tapPoleId = p.tapPoleId;
              if (tapBranchIndex == null && p.tapBranchIndex != null) {
        ...
        if (tapBranchIndex == null && tapPoleId != null) {
          for (final p in poles) {
            final n = p.poleNumber.trim();
            if (p.tapPoleId == tapPoleId &&
                n.startsWith('$rootEarly/') &&
                p.tapBranchIndex != null) {
              tapBranchIndex = p.tapBranchIndex;
              break;
            }
          }
    ...
    if (tapPoleId == null) {
      final db = ref.read(drift_db.databaseProvider);
      final linePoles = await db.getPolesByLine(lineId);
      final rootList = linePoles.where((p) => p.poleNumber.trim() == rootEarly).toList();
    ...
    final nextPoleNumber = '$rootEarly/${suffixEarly + 1}';
    final db = ref.read(drift_db.databaseProvider);
    final linePoles = await db.getPolesByLine(lineId);
    if (!mounted) return;
    _closeObjectProperties();
    // Контекст отпайки для FAB до и после диалога (в т.ч. если диалог закрыли без сохранения).
    setState(() {
      _currentTapRoot = rootEarly;
      _currentTapBranchIndex = tapBranchIndex;
      _currentTapAnchorPoleId = tapPoleId;
    });
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
  ...
  Future<void> _showCreatePoleDialog() async {
    ...
    if (_currentTapRoot != null && _currentTapRoot!.isNotEmpty) {
      tapPoleIdForFab = _currentTapAnchorPoleId;
      final maxSuffix = await _maxTapSuffixForRoot(
        selectedLineId,
        _currentTapRoot!,
        linePoles,
      );
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) => CreatePoleDialog(
        ...
        existingPolesCount: existingPolesCount,
        initialLineVoltageKv: _voltageKvForPowerLine(selectedLineId),
      ),
    );
  ...
  void _startLineFormation(int poleId, int lineId) async {
    ...
    // Актуальная точка и центрирование (поток GPS уже идёт с инициализации карты).
    try {
      final position = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.best,
      ...
      if (mounted) {
        try {
          ...
          if (kDebugMode) print('Ошибка центрирования при старте навигатора: $e');
        }
      }
    } catch (e) {
      if (kDebugMode) print('Ошибка получения местоположения: $e');
  ...
  void _stopLineFormation() {
    // Поток GPS не отменяем — маркер «я» на карте продолжает обновляться.
