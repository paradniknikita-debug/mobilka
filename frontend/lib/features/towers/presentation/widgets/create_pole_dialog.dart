...
class CreatePoleDialog extends ConsumerStatefulWidget {
  ...
  /// Номинал линии (кВ), если уже известен — сразу фильтруем марки оборудования до загрузки ЛЭП с API.
  final double? initialLineVoltageKv;
...
class _CreatePoleDialogState extends ConsumerState<CreatePoleDialog> {
  ...
  /// Если [CreatePoleDialog.tapPoleId] не передан (якорь не найден на карте/в БД), id якоря по корню номера «N/…» из списка опор линии.
  int? _resolvedTapAnchorId;

  /// Эффективный id отпаечной опоры для ветки и API (явный из виджета или восстановленный по номеру).
  int? get _effectiveTapPoleId => widget.tapPoleId ?? _resolvedTapAnchorId;
  ...
  void _clearAutofillEquipment() {
    setState(() {
      _pendingEquipment.removeWhere((e) =>
          e.categoryTitle != null && _autofillCategories.contains(e.categoryTitle));
      _autofillApplied = false;
      for (var i = 0; i < _equipmentCategories.length; i++) {
        final catTitle = _equipmentCategories[i].title;
        final eqType = EquipmentReferenceData.categoryToEquipmentType[catTitle];
        final hasPending = _pendingEquipment.any((e) => e.equipmentType == eqType);
        final hasLoaded = _loadedEquipment.any((e) {
          final t = e.equipmentType.trim().toLowerCase();
          final catType = eqType?.trim().toLowerCase();
          return catType != null && (t == catType || t.contains(catType) || catType.contains(t));
        });
        _equipmentInstalled[i] = hasPending || hasLoaded;
      }
    });
  }
  ...
  void _applyConductorBrand(String? brand) {
    _conductorType = brand;
    if (brand == null || brand.isEmpty) return;
    final upper = brand.toUpperCase();
    final m = RegExp(r'(\d+)$').firstMatch(upper);
    if (m != null) {
      final sec = m.group(1);
      if (sec != null && PoleReferenceData.conductorSections.contains(sec)) {
        _conductorSection = sec;
      }
    }
    if (upper.startsWith('AC-') || upper.startsWith('АС-')) {
      _conductorMaterial = 'сталь-алюминий';
    }
  }
  ...
  void initState() {
    super.initState();
    _lineVoltageKv = widget.initialLineVoltageKv;
    ...
    // «Точка отпайки» — только у опоры на магистрали, от которой отходит ветка.
    // Первая опора новой отпайки (номер N/1, 2/1, …) — обычная опора ветки, не is_tap.
  ...
  Future<void> _loadLineVoltage() async {
    try {
      final apiService = ref.read(apiServiceProvider);
      final line = await apiService.getPowerLine(widget.lineId);
      if (!mounted) return;
      setState(() {
        final v = line.voltageLevel;
        if (v != null && v > 0) {
          _lineVoltageKv = v;
        } else {
          _lineVoltageKv = _lineVoltageKv ?? widget.initialLineVoltageKv;
        }
      });
      return;
    } catch (_) {}
    try {
      ...
      final line = await db.getPowerLine(widget.lineId);
      if (!mounted) return;
      setState(() {
        final v = line?.voltageLevel;
        if (v != null && v > 0) {
  ...
  void _applyLoadedPoleToState(Pole pole, List<Equipment> equipmentList) {
    ...
    setState(() {
      ...
      _notes = pole.notes;
      _conductorType = pole.conductorType ?? PoleReferenceData.defaultConductorType;
      _conductorMaterial =
          pole.conductorMaterial ?? PoleReferenceData.defaultConductorMaterial;
      _conductorSection =
          pole.conductorSection ?? PoleReferenceData.defaultConductorSection;
  ...
  Future<void> _loadTapBranchesForLine() async {
    try {
      ...
      List<Pole> poles = [];
      try {
        poles = await apiService.getPoles(widget.lineId);
      } catch (_) {
        poles = [];
      }
      if (poles.isEmpty) {
        try {
          ...
          for (final r in rows) {
            poles.add(Pole(
              ...
              xPosition: r.xPosition ?? 0.0,
              yPosition: r.yPosition ?? 0.0,
              poleType: r.poleType ?? '',
              height: r.height,
              ...
              notes: r.notes,
              cardComment: r.cardComment,
              cardCommentAttachment: r.cardCommentAttachment,
          ...
          // нет локальных опор линии
        }
      }
      if (!mounted) return;
      int? anchorForBranch = widget.tapPoleId;
      if (anchorForBranch == null &&
          widget.tapBranchIndex != null &&
          widget.initialPoleNumber != null &&
          widget.initialPoleNumber!.contains('/')) {
        final root = widget.initialPoleNumber!.split('/').first.trim();
        for (final p in poles) {
          if (p.poleNumber.trim() == root) {
            anchorForBranch = p.id;
            break;
          }
        }
      }
      ...
      // Если диалог открыт как "продолжить текущую отпайку", принудительно
      // сохраняем эту ветку в списке (даже если API временно вернул неполные данные).
      if (anchorForBranch != null && widget.tapBranchIndex != null) {
        final forcedKey = '$anchorForBranch:${widget.tapBranchIndex}';
        final exists = branches.any((b) => b.key == forcedKey);
        if (!exists) {
          final tapLabel = tapPoleNames[anchorForBranch] ?? 'опора $anchorForBranch';
          branches.add(MapEntry(forcedKey, 'Отпайка от $tapLabel — ветка ${widget.tapBranchIndex}'));
        }
      }
      final allowedValues = branches.map((e) => e.key).toSet();
      setState(() {
        _resolvedTapAnchorId = widget.tapPoleId == null ? anchorForBranch : null;
        ...
        if (anchorForBranch != null && widget.tapBranchIndex != null) {
          _branchSelection = '$anchorForBranch:${widget.tapBranchIndex}';
        }
  ...
  Future<void> _loadPoleNumberSuggestion() async {
    ...
    try {
      ...
      Iterable<String> filtered = numbers;
      String? targetRoot;
      if (widget.initialPoleNumber != null && widget.initialPoleNumber!.contains('/')) {
        targetRoot = widget.initialPoleNumber!.split('/').first.trim();
      } else if (widget.tapPoleId != null && widget.initialPoleNumber != null && widget.initialPoleNumber!.contains('/')) {
        targetRoot = widget.initialPoleNumber!.split('/').first.trim();
      }
      if (targetRoot != null && targetRoot.isNotEmpty) {
        filtered = numbers.where((n) => n.trim().startsWith('$targetRoot/'));
      } else {
        filtered = numbers.where((n) => !n.contains('/'));
      }
      final filteredList = filtered.toList();
      final sug = PoleNumberSuggestion.suggestNext(filteredList.isEmpty ? numbers : filteredList);
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (widget.startNewTap && widget.tapPoleId != null) {
        ...
        startNewTap = true;
        branchType = 'tap';
      } else if (_effectiveTapPoleId != null && widget.tapBranchIndex != null) {
        tapPoleId = _effectiveTapPoleId;
      ...
      final createdPole = await apiService.createPole(
        widget.lineId,
        poleData,
        fromPoleId: _effectiveTapPoleId,
      );
              ...
              Flexible(
                ...
                children: [
                  ...
                  Expanded(
                              ...
                              Switch(
                                ...
                                onChanged: widget.existingPolesCount == 0
                                    ...
                                    : (v) {
                                        ...
                                        } else if (_autofillApplied) {
                                          _clearAutofillEquipment();
                                        }
                                      },
                      ...
                      if (_showBranchChoice && !(_effectiveTapPoleId != null && widget.startNewTap)) ...[
                            ...
                            return Container(
                              ...
                              child: DropdownButtonHideUnderline(
                                child: DropdownButton<String?>(
                                  ...
                                  items: dropdownItems,
                                  onChanged: (_effectiveTapPoleId != null && widget.tapBranchIndex != null)
                                      ? null
                  ...
                  const SizedBox(width: 8),
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items: PoleReferenceData.conductorTypes
                            .map((e) => DropdownMenuItem<String>(
                                  value: e,
                                  child: Text(e),
                                ))
                            .toList(),
                        onChanged: (v) => setState(() => _applyConductorBrand(v)),
                      ),
                      const SizedBox(height: 20),
