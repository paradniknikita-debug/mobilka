...
class ProfilePage extends ConsumerWidget {
          ...
          _profileCard(
            child: Column(
              children: [
                ...
                ListTile(
                  leading: const Icon(Icons.brightness_6, color: PatrolColors.accent),
                  title: const Text('Тема приложения', style: TextStyle(color: PatrolColors.textPrimary)),
                  subtitle: Text(
                    ref.watch(themeModeProvider) == ThemeMode.dark ? 'Тёмная' : 'Светлая',
                    style: const TextStyle(color: PatrolColors.textSecondary),
                  ),
                  trailing: Switch(
            ...
            child: Column(
              children: [
                ...
                ListTile(
                  ...
                  title: const Text('Синхронизировать справочник марок', style: TextStyle(color: PatrolColors.textPrimary)),
                  subtitle: const Text('Обновить локальный кэш справочника из основной БД', style: TextStyle(color: PatrolColors.textSecondary)),
                  onTap: () => _syncCatalogDictionaries(context, ref),
                ),
                ListTile(
  ...
  Future<void> _syncCatalogDictionaries(BuildContext context, WidgetRef ref) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Справочник обновлён: ${rows.length} записей', style: const TextStyle(color: PatrolColors.background)),
          backgroundColor: PatrolColors.statusSynced,
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Ошибка синхронизации справочника: ${e.toString()}', style: const TextStyle(color: PatrolColors.background)),
