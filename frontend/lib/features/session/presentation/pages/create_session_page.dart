...
class _CreateSessionPageState extends ConsumerState<CreateSessionPage> {
  ...
  Future<Position?> _captureBestPositionSampled({
    ...
    try {
      sub = Geolocator.getPositionStream(
        ...
        if (mounted) {
          setState(() {
            _gpsSamplesTaken = samples;
          });
        }
        final better = best == null || pos.accuracy < best!.accuracy;
        if (better) {
          best = pos;
        }
        final reachedTarget = (best?.accuracy ?? 9999) <= targetAccuracyMeters;
        final overSamples = samples >= maxSamples;
        final overTime = DateTime.now().difference(startedAt) >= maxDuration;
        if (reachedTarget || overSamples || overTime) {
          finish();
        }
      }, onError: (_) {
        finish();
      ...
      // fallback таймер (если поток редкий/завис)
      Future.delayed(maxDuration + const Duration(seconds: 1), finish);
  ...
  String _accuracyLabel() {
    ...
    if (_accuracyMeters! <= 5) return '±5 м';
    return 'хуже ±5 м (факт ±${_accuracyMeters!.round()} м)';
  }
  ...
  Future<void> _submit() async {
    ...
    if (_accuracyMeters != null) {
      await prefs.setDouble(AppConfig.activeSessionAccuracyKey, _accuracyMeters!);
    } else {
              ...
              child: Column(
                ...
                children: [
                  _GpsBlock(
                    ...
                    gpsSamplesTaken: _gpsSamplesTaken,
                    loading: _gpsLoading,
                    warning: _gpsWarning,
                    onRetry: _captureGps,
          ...
          if (loading)
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                const Text(
                  'Определение координат (автопоиск лучшей точности)...',
                  style: TextStyle(color: PatrolColors.textSecondary, fontSize: 14),
                ),
                if (gpsSamplesTaken > 0) ...[
                  const SizedBox(width: 10),
                  Text(
                    'замер $gpsSamplesTaken/8',
                    style: const TextStyle(color: PatrolColors.textSecondary, fontSize: 12),
                  ),
                ],
              ],
