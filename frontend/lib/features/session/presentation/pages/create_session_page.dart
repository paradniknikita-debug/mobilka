...
class _CreateSessionPageState extends ConsumerState<CreateSessionPage> {
  ...
  Future<void> _clearActiveSessionPrefs() async {
  ...
  /// true, если в приложении есть незавершённая активная сессия.
  Future<bool> _hasUnfinishedActiveSession() async {
    final prefs = ref.read(prefsProvider);
    final lineId = prefs.getInt(AppConfig.activeSessionPowerLineIdKey);
  ...
  Future<void> _submit() async {
    if (await _hasUnfinishedActiveSession()) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Сначала завершите текущий обход, затем создавайте новый.'),
