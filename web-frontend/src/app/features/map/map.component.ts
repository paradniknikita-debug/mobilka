...
export class MapComponent implements OnInit, OnDestroy {
  ...
  private equipmentLabels: L.Layer[] = []; // Подписи линейного оборудования
    ...
    geoJson.features.forEach((feature: GeoJSONFeature) => {
      if (feature.geometry.type === 'Point') {
        ...
        (marker as any).poleData = {
          ...
          poleNumber: feature.properties['pole_number'] || 'не указано'
        };
  ...
  private normalizePoleRef(value: any): string {
    return String(value ?? '').trim().toLowerCase().replace(/\s+/g, ' ');
  }

  private matchesPoleRef(refRaw: any, pole: GeoJSONFeature): boolean {
    const ref = this.normalizePoleRef(refRaw);
    if (!ref) return false;
    const id = Number(pole.properties?.['id']);
    const poleNumber = this.normalizePoleRef(pole.properties?.['pole_number']);
    return ref === String(id).toLowerCase() || ref === poleNumber;
  }

  /** Явное направление к соседней опоре используем только для коммутационного оборудования (как во Flutter). */
  private usesDirectionalPoleRef(equipmentTypeRaw: any): boolean {
    const type = String(equipmentTypeRaw ?? '').trim().toLowerCase();
    return type.includes('разъедин') ||
      type.includes('disconnector') ||
      type === 'grounding_switch' ||
      type.includes('grounding') ||
      type === 'zn' ||
      type.includes('зн') ||
      type === 'breaker' ||
      type.includes('выключател') ||
      type === 'recloser' ||
      type.includes('реклоузер');
  }
  ...
  private offsetAlongAndPerpendicular(
    from: L.LatLng,
    to: L.LatLng,
    base: L.LatLng,
    slot: number
  ...
  private renderEquipmentBetweenPoles(
    ...
    powerLinesList.forEach((pl) => {
      ...
      const adjacency = new Map<number, { p1: GeoJSONFeature; p2: GeoJSONFeature }[]>();
      const segmentKeys = new Set<string>();
      branchEntries.forEach(([branchKey, branchPoles]) => {
        ...
        if (keyParts[1] !== 'main') {
          ...
          if (Number.isFinite(anchorId)) {
            ...
            if (firstId !== anchorId) {
              ...
              if (anchorFeat) poles = [anchorFeat, ...poles];
            }
          }
        }
        for (let i = 0; i < poles.length - 1; i++) {
          ...
          adjacency.get(id1)!.push(seg);
          adjacency.get(id2)!.push(seg);
        }
      });
      ...
      const allPolesForLine = Array.from(
        new Map(
          polesFeatures
            .filter((f: GeoJSONFeature) => this.lineIdFromProps(f.properties as Record<string, any> | undefined) === pl.id)
            .map((f: GeoJSONFeature) => [Number(f.properties?.['id']), f] as [number, GeoJSONFeature])
        ).values()
      );

      allPolesForLine.forEach((poleFeature) => {
        const poleId = Number(poleFeature.properties?.['id']);
        if (!Number.isFinite(poleId)) return;
        const candidates = adjacency.get(poleId) || [];
        if (!candidates.length) return;
        const eqList = (this.equipmentByPoleId.get(poleId) || [])
          .map((eq) => ({ eq, iconKey: this.getLineEquipmentIconKey(eq) }))
        ...
        eqList.forEach((eq, idx) => {
          ...
          if (
            ...
            directionRef != null &&
            String(directionRef).trim().length > 0
          ) {
          ...
          const rot = this.getIconRotationDeg(iconKey, modelAngleDeg);
          ...
          const eqName = String((eq as any).name ?? '').trim();
          (marker as any).equipmentLabelText = eqName;
          (marker as any).poleFeature = poleFeature;
          (marker as any).lineId = pl.id;
          marker.on('click', () => {
    ...
    geoJson.features.forEach((feature: GeoJSONFeature) => {
      if (feature.geometry.type === 'Point') {
        ...
        const marker = L.marker(latlng, {
          ...
          <strong>Отпайка ${feature.properties['tap_number'] || 'не указано'}</strong><br>
          Тип: ${feature.properties['tap_type'] || 'не указано'}<br>
          Напряжение: ${feature.properties['voltage_level'] || 'не указано'} кВ
        `);
  ...
  clearLayers(): void {
    ...
    this.equipmentLabels.forEach(label => this.map?.removeLayer(label));
  ...
  updatePoleLabels(): void {
    ...
    this.updateEquipmentLabels();
  }

  private updateEquipmentLabels(): void {
    if (!this.map) return;
    this.equipmentLabels.forEach((label) => this.map?.removeLayer(label));
    this.equipmentLabels = [];
    if (!(this.currentZoom >= 15 && this.currentZoom <= 20)) return;
    this.equipmentGeoJsonMarkers.forEach((marker) => {
      const text = String((marker as any).equipmentLabelText ?? '').trim();
      if (!text) return;
      const latlng = marker.getLatLng();
      const label = L.marker(latlng, {
        icon: L.divIcon({
          className: 'equipment-label',
          html: `<div class="equipment-label-text">${text}</div>`,
          iconSize: [140, 20],
          iconAnchor: [70, -8]
        }),
        interactive: false,
  ...
  private updateEquipmentVisibility(): void {
    ...
    this.equipmentLabels.forEach(m => {
      if (show) { if (!this.map!.hasLayer(m)) m.addTo(this.map!); }
      else { this.map!.removeLayer(m); }
    });
