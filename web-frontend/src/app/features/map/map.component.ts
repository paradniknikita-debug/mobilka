...
export class MapComponent implements OnInit, OnDestroy {
  ...
  renderPowerLines(geoJson: any, polesFeatures: GeoJSONFeature[] = []): void {
    ...
    polesFeatures.forEach((f: GeoJSONFeature) => {
      ...
      const k = this.getPoleVisualBranchKey(Number(lineId), f.properties ?? {});
    ...
    // Как во Flutter: при наличии опор строим линии только по ним — без «ломаных» LineString из GeoJSON
    // при неполных branch/tap полях (ложные диагонали между ветками).
    if (polesByBranch.size > 0) {
      this.renderPowerLinesFromPolesOnly(polesById, polesByBranch);
      return;
    }
    ...
    geoJson.features.forEach((feature: GeoJSONFeature) => {
      ...
      let branchKey = lineId != null ? this.getPoleVisualBranchKey(Number(lineId), feature.properties ?? {}) : null;
      // У фичи LineString часто нет pole_number (только tap_branch_index) — ключ не совпадает с точками (r:корень).
      // Тогда подбираем ключ по любой опоре ветки с тем же tap_pole_id и tap_branch_index (старый API / кэш).
      if (branchKey && !polesByBranch.has(branchKey) && feature.properties?.['branch_type'] === 'tap') {
        const tpid = feature.properties?.['tap_pole_id'];
        const tbi = feature.properties?.['tap_branch_index'];
        if (lineId != null && tpid != null && tbi != null) {
          for (const pf of polesFeatures) {
            if (pf.geometry?.type !== 'Point') continue;
            const lid = pf.properties?.['line_id'] ?? pf.properties?.['power_line_id'];
            if (lid == null || Number(lid) !== Number(lineId)) continue;
            if (Number(pf.properties?.['tap_pole_id']) !== Number(tpid)) continue;
            if (Number(pf.properties?.['tap_branch_index']) !== Number(tbi)) continue;
            const k = this.getPoleVisualBranchKey(Number(lineId), pf.properties ?? {});
            if (polesByBranch.has(k)) {
              branchKey = k;
              break;
            }
          }
      ...
      if (branchKey && polesByBranch.has(branchKey)) {
        ...
        const tapPoleIdRaw = feature.properties?.['tap_pole_id'] ?? poles[0]?.properties?.['tap_pole_id'] ?? null;
        const tapId = tapPoleIdRaw != null ? Number(tapPoleIdRaw) : null;
  ...
  /** Напряжение линии по id (для цвета), как во Flutter. */
  private voltageKvForPowerLine(lineId: number): number {
    const list = this.mapData?.powerLinesList;
    if (!list?.length) return NaN;
    const pl = list.find((p) => Number(p.id) === lineId);
    if (!pl || pl.voltage_level == null) return NaN;
    const v = Number(pl.voltage_level);
    return Number.isFinite(v) ? v : NaN;
  }

  /**
   * ЛЭП только по опорам (магистраль и отпайки), без геометрии LineString из коллекции.
   * Сегмент якорь → первая отпаечная опора и ветка из одной опоры — как во Flutter.
   */
  private renderPowerLinesFromPolesOnly(
    polesById: Map<number, GeoJSONFeature>,
    polesByBranch: Map<string, GeoJSONFeature[]>
  ): void {
    if (!this.map) return;

    const latLng = (f: GeoJSONFeature): L.LatLngExpression => {
      const c = f.geometry!.coordinates as number[];
      return [Number(c[1]), Number(c[0])];
    };

    const addPolyline = (pts: L.LatLngExpression[], isTap: boolean, lineId: number) => {
      if (pts.length < 2) return;
      const vEff = this.voltageKvForPowerLine(lineId);
      const lineColor = colorForVoltageKv(Number.isFinite(vEff) ? vEff : null);
    ...
    polesByBranch.forEach((poleList, branchKey) => {
      if (poleList.length === 0) return;
      const lineId = Number(branchKey.split(':')[0]);
      if (!Number.isFinite(lineId)) return;

      const tapBranch = branchKey !== `${lineId}:main`;
      const tapId = this.inferTapPoleIdForBranch(poleList);
      const useEquipmentSegments = poleList.length >= 2;

      if (poleList.length === 1) {
        if (tapBranch && tapId != null) {
          const base = polesById.get(tapId);
          if (base?.geometry?.type === 'Point') {
            addPolyline([latLng(base), latLng(poleList[0])], true, lineId);
          }
        }
        return;
      }
      ...
      if (tapBranch && tapId != null && useEquipmentSegments) {
        ...
        for (let i = 0; i < poleList.length - 1; i++) {
          addPolyline([latLng(poleList[i]), latLng(poleList[i + 1])], true, lineId);
        }
      } else if (tapBranch && tapId != null && !useEquipmentSegments) {
        const pts: L.LatLngExpression[] = [];
        const base = polesById.get(tapId);
        if (base?.geometry?.type === 'Point') pts.push(latLng(base));
        poleList.forEach((p) => pts.push(latLng(p)));
        addPolyline(pts, true, lineId);
      } else {
        addPolyline(poleList.map((p) => latLng(p)), false, lineId);
      }
    });
  }

  /** Якорь только из tap_pole_id — префикс 1/x не задаёт магистральную опору. */
  private inferTapPoleIdForBranch(poleList: GeoJSONFeature[]): number | null {
    if (!poleList.length) return null;
    for (const p of poleList) {
      const tid = p.properties?.['tap_pole_id'];
  ...
  /**
   * Устойчивый ключ визуальной ветки. «Отпайка от отпайки» — ≥3 сегментов в номере;
   * ключ sub:префикс (без последнего сегмента), чтобы не сливать с веткой N/M.
   */
  private getPoleVisualBranchKey(lineId: number, props: Record<string, any> | null | undefined): string {
    const poleNumber = String(props?.['pole_number'] ?? '').trim();
    const segments = poleNumber.split('/').map((p) => p.trim()).filter(Boolean);
    const hasSlash = segments.length >= 2;
    const tapOrdinal = segments[0] ?? '';
    const tapPoleIdRaw = props?.['tap_pole_id'];
    const tapPoleIdNum = Number(tapPoleIdRaw);
    const tapBranchIndexRaw = props?.['tap_branch_index'];
    const tapBranchIndex = Number(tapBranchIndexRaw);
    const nestedPathPrefix =
      segments.length >= 3 ? segments.slice(0, segments.length - 1).join('/') : null;

    if (hasSlash) {
      if (tapPoleIdRaw != null && Number.isFinite(tapPoleIdNum)) {
        if (nestedPathPrefix) {
          if (Number.isFinite(tapBranchIndex)) {
            return `${lineId}:tap:${tapPoleIdNum}:sub:${nestedPathPrefix}:b:${tapBranchIndex}`;
          }
          return `${lineId}:tap:${tapPoleIdNum}:sub:${nestedPathPrefix}`;
        }
        if (Number.isFinite(tapBranchIndex)) {
          return `${lineId}:tap:${tapPoleIdNum}:b:${tapBranchIndex}`;
        }
        if (tapOrdinal) {
          return `${lineId}:tap:${tapPoleIdNum}:n:${tapOrdinal}`;
        }
        return `${lineId}:tap:${tapPoleIdNum}:x`;
      }
      if (nestedPathPrefix) {
        if (Number.isFinite(tapBranchIndex)) {
    ...
    const tapPoleId = Number(tapPoleIdRaw);
    if (!Number.isFinite(tapPoleId)) {
      return `${lineId}:main`;
    }
  ...
  private renderEquipmentBetweenPoles(
    ...
    polesFeatures.forEach((f: GeoJSONFeature) => {
      ...
      const k = this.getPoleVisualBranchKey(lid, f.properties ?? {});
  ...
  renderPoleSequence(geoJson: any): void {
    ...
    // Для отпаек ключом группы служит в первую очередь корень номера опоры (1/*, 2/*),
    // а затем tap_branch_index. Это защищает от склейки разных веток,
    // если backend временно вернул одинаковый/null tap_branch_index.
    ...
    geoJson.features.forEach((feature: GeoJSONFeature) => {
      ...
      if (lineId != null && feature.properties['sequence_number'] != null) {
        const k = this.getPoleVisualBranchKey(Number(lineId), feature.properties ?? {});
