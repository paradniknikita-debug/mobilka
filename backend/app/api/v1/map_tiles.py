    ...
    for pole in poles:
        ...
        if longitude is not None and latitude is not None:
            ...
            properties = {
                ...
                "notes": str(pole.notes) if pole.notes else None,
            }
    ...
    for substation in substations:
        ...
        if longitude is not None and latitude is not None:
            ...
            properties = {
                ...
                "mrid": str(getattr(substation, "mrid", "") or ""),
...
def _line_sort_key(p):
    ...
    if sn is not None:
        return (0, sn, (p.pole_number or ""))
    return (1, _pole_order_from_number(getattr(p, "pole_number", "") or ""), (p.pole_number or ""))


def _pole_order_from_number(pole_number: str) -> int:
    t = (pole_number or "").strip()
    if not t:
        return 1 << 30
    if "/" not in t:
        return int(t) if t.isdigit() else (1 << 30)
    parts = t.split("/")
    if len(parts) < 2:
        return 1 << 30
...
async def _get_equipment_geojson_impl(db: AsyncSession):
    ...
    for power_line in power_lines:
        rendered_equipment_ids: set[int] = set()

        def _target_ref(eq: Equipment) -> str:
            return str(getattr(eq, "parent_main_equipment_pole_ref", "") or "").strip().lower()

        def _matches_target_pole(eq: Equipment, pole: Pole) -> bool:
            ref = _target_ref(eq)
            if not ref:
                return False
            pid = str(getattr(pole, "id", "") or "").strip().lower()
            pn = str(getattr(pole, "pole_number", "") or "").strip().lower()
            return ref == pid or ref == pn

        def _append_equipment_feature(
            eq: Equipment,
            from_pole: Pole,
            to_pole: Pole,
            x1: float,
            ...
            icon_key = _equipment_icon_key(eq.equipment_type, eq.name)
            if not icon_key:
                return
            lng = x1 + (x2 - x1) * t
            lat = y1 + (y2 - y1) * t
            if from_pole.id <= to_pole.id:
                x_lo, y_lo, x_hi, y_hi = x1, y1, x2, y2
                from_id, to_id = from_pole.id, to_pole.id
            else:
                x_lo, y_lo, x_hi, y_hi = x2, y2, x1, y1
                from_id, to_id = to_pole.id, from_pole.id
            ...
            features.append({
                ...
                "properties": {
                    ...
                    "equipment_id": eq.id,
                    "mrid": getattr(eq, "mrid", None),
                    "pole_number": getattr(to_pole, "pole_number", None),
                    "condition": getattr(eq, "condition", None),
                    "notes": getattr(eq, "notes", None),
                    "has_defect": _has_equipment_defect(eq),
                    "defect_criticality": _normalize_criticality(getattr(eq, "criticality", None)),
                },
            })
            if getattr(eq, "id", None) is not None:
                rendered_equipment_ids.add(int(eq.id))
        ...
        for i in range(len(main_poles) - 1):
            ...
            try:
                ...
                continue
            # Оборудование на p1: показываем только при явном направлении к p2.
            for e in (list(getattr(p1, "equipment", None) or [])):
                eid = getattr(e, "id", None)
                if eid is not None and int(eid) in rendered_equipment_ids:
                    continue
                if _matches_target_pole(e, p2):
                    _append_equipment_feature(e, p1, p2, x1, y1, x2, y2, 0.2)

            # Оборудование на p2: legacy-поведение (к предыдущей опоре), либо явное направление к p1.
            eq_list = list(getattr(p2, "equipment", None) or [])
            legacy_visible = [
                e for e in eq_list
                if _equipment_icon_key(e.equipment_type, e.name) is not None and not _target_ref(e)
            ]
            for j, e in enumerate(legacy_visible):
                eid = getattr(e, "id", None)
                if eid is not None and int(eid) in rendered_equipment_ids:
                    continue
                t = 0.8 if len(legacy_visible) == 1 else 0.6 + (0.3 * (j / max(1, len(legacy_visible) - 1)))
                _append_equipment_feature(e, p1, p2, x1, y1, x2, y2, t)
            for e in eq_list:
                eid = getattr(e, "id", None)
                if eid is not None and int(eid) in rendered_equipment_ids:
                    continue
                if _matches_target_pole(e, p1):
                    _append_equipment_feature(e, p1, p2, x1, y1, x2, y2, 0.8)
        ...
        for tpid in tap_pole_ids:
            ...
            if not tap_pole:
                continue
            branch_poles = [p for p in poles_list if getattr(p, "tap_pole_id", None) == tpid]
            branch_poles.sort(key=lambda p: (
                0 if getattr(p, "sequence_number", None) is not None else 1,
                getattr(p, "sequence_number", None) if getattr(p, "sequence_number", None) is not None else _pole_order_from_number(getattr(p, "pole_number", "") or ""),
                getattr(p, "id", 0),
            ))
...
async def get_spans_geojson(
    ...
    result = await db.execute(
        ...
        .options(
            ...
            selectinload(Span.from_connectivity_node).selectinload(ConnectivityNode.pole).selectinload(Pole.location).selectinload(Location.position_points),
            selectinload(Span.to_connectivity_node).selectinload(ConnectivityNode.pole).selectinload(Pole.position_points),
            selectinload(Span.to_connectivity_node).selectinload(ConnectivityNode.pole).selectinload(Pole.location).selectinload(Location.position_points),
    ...
    for span in spans:
        ...
        if span.from_connectivity_node and span.to_connectivity_node:
            ...
            try:
                ...
                if pole:
                    from_longitude, from_latitude = _pole_coords_from_position_point(pole)
            except (AttributeError, IndexError, TypeError):
                pass
            ...
            try:
                ...
                if pole:
                    to_longitude, to_latitude = _pole_coords_from_position_point(pole)
            except (AttributeError, IndexError, TypeError):
                pass
