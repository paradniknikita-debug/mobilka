...
def _pole_orm_to_dict(pole) -> dict:
    ...
    out = {
        ...
        "construction": _get_attr_safe(pole, "construction"),
        "rated_voltage": _get_attr_safe(pole, "rated_voltage"),
...
def _power_line_orm_to_dict(pl) -> dict:
    ...
    return {
        ...
        "dispatcher_name": _get_attr_safe(pl, "dispatcher_name"),
        "branch_name": _get_attr_safe(pl, "branch_name"),
        "region_name": _get_attr_safe(pl, "region_name"),
        "region_uid": _get_attr_safe(pl, "region_uid"),
        "balance_ownership": _get_attr_safe(pl, "balance_ownership"),
        "parent_object_ref": _get_attr_safe(pl, "parent_object_ref"),
        "alcs_ref": _get_attr_safe(pl, "alcs_ref"),
...
async def create_power_line(
    ...
    region_uid = power_line_dict.get("region_uid")
    if not region_uid:
        power_line_dict["region_uid"] = "c3d4e5f6-7890-1234-cdef-345678901234"
    ...
    print(f"  description: {power_line_dict.get('description')}")
    print(f"  created_by: {current_user.id}")
    print(f"  Все поля power_line_dict (до удаления description): {power_line_dict}")
    
    power_line_dict_clean = dict(power_line_dict)
    print(f"  Все поля power_line_dict: {power_line_dict_clean}")
...
async def update_pole(
    ...
    old_snapshot = {
        ...
        "notes": pole.notes,
        "structural_defect": pole.structural_defect,
        "structural_defect_criticality": pole.structural_defect_criticality,
        "is_tap_pole": pole.is_tap_pole,
        "branch_type": pole.branch_type,
        "tap_pole_id": pole.tap_pole_id,
        "tap_branch_index": pole.tap_branch_index,
        "conductor_type": pole.conductor_type,
        "conductor_material": pole.conductor_material,
        "conductor_section": pole.conductor_section,
        "card_comment": pole.card_comment,
        "card_comment_attachment": pole.card_comment_attachment,
