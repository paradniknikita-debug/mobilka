...
async def create_equipment(
    ...
    identified_object_description = data.get("identified_object_description") or data.get(
        "identifiedObjectDescription"
    )
    nameplate = data.get("nameplate")
    psr_subtype = data.get("psr_subtype") or data.get("psrSubtype")
    installation_display_name = data.get("installation_display_name") or data.get(
        "installationDisplayName"
    )
    tm_code = data.get("tm_code") or data.get("tmCode")
    object_subtype = data.get("object_subtype") or data.get("objectSubtype")
    pole_count = data.get("pole_count") or data.get("poleCount")
    parent_object_ref = data.get("parent_object_ref") or data.get("parentObjectRef")
    parent_main_equipment_pole_ref = data.get("parent_main_equipment_pole_ref") or data.get(
    ...
    own_trip_time_sec = data.get("own_trip_time_sec") or data.get("ownTripTimeSec")
    emergency_current_a = data.get("emergency_current_a") or data.get("emergencyCurrentA")
    continuous_current_a = data.get("continuous_current_a") or data.get("continuousCurrentA")
    arrester_type = data.get("arrester_type") or data.get("arresterType")
    ...
    db_equipment = Equipment(
        ...
        retained=retained,
        identified_object_description=identified_object_description,
        nameplate=nameplate,
        psr_subtype=psr_subtype,
        installation_display_name=installation_display_name,
        tm_code=tm_code,
        object_subtype=object_subtype,
        pole_count=pole_count,
        parent_object_ref=parent_object_ref,
        parent_main_equipment_pole_ref=parent_main_equipment_pole_ref,
        nominal_voltage_kv=nominal_voltage_kv,
        nominal_breaking_current_ka=nominal_breaking_current_ka,
        own_trip_time_sec=own_trip_time_sec,
        emergency_current_a=emergency_current_a,
        continuous_current_a=continuous_current_a,
        arrester_type=arrester_type,
            ...
            db.add(
                ChangeLog(
                    ...
                    payload={
                        ...
                        "condition": db_equipment.condition,
                        "notes": db_equipment.notes,
                        "nameplate": db_equipment.nameplate,
                        "rated_current": db_equipment.rated_current,
                        "i_th": db_equipment.i_th,
                        "ip_max": db_equipment.ip_max,
                        "t_th": db_equipment.t_th,
                        "defect": defect,
                        "criticality": criticality,
