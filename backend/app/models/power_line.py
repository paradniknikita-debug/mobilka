...
class PowerLine(Base, ConnectivityNodeContainer):
    ...
    dispatcher_name = Column(String(255), nullable=True)
    branch_name = Column(String(255), nullable=True)
    region_name = Column(String(255), nullable=True)
    region_uid = Column(String(36), nullable=True, default="c3d4e5f6-7890-1234-cdef-345678901234")
    balance_ownership = Column(String(255), nullable=True)
    parent_object_ref = Column(String(255), nullable=True)
    alcs_ref = Column(String(255), nullable=True)
...
class Equipment(Base):
    ...
    # CIM IdentifiedObject.description — номер единицы оборудования (инв. / учётный)
    identified_object_description = Column(String(255), nullable=True)
    # CIM me:Equipment.nameplate — марка с шильдика
    nameplate = Column(String(255), nullable=True)
    # Подтип ПСР: retractable (выкатной) | sectionalizer (отделитель) — влияет на PSRType в выгрузке
    psr_subtype = Column(String(40), nullable=True)
    # Название электроустановки (для отображения; в CIM обычно тот же контейнер, что и ЛЭП)
    installation_display_name = Column(String(255), nullable=True)
    # Дополнительные карточечные атрибуты (выключатель/реклоузер/разрядник)
    tm_code = Column(String(100), nullable=True)
    object_subtype = Column(String(100), nullable=True)
    pole_count = Column(Integer, nullable=True)
    parent_object_ref = Column(String(255), nullable=True)
    parent_main_equipment_pole_ref = Column(String(255), nullable=True)
    nominal_voltage_kv = Column(Float, nullable=True)
    nominal_breaking_current_ka = Column(Float, nullable=True)
    own_trip_time_sec = Column(Float, nullable=True)
    emergency_current_a = Column(Float, nullable=True)
    continuous_current_a = Column(Float, nullable=True)
    arrester_type = Column(String(40), nullable=True)
