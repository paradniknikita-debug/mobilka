...
async def find_previous_pole_in_tap_branch(
    ...
    if suffix == 1:
        ...
        return root_pole
    # Иначе предыдущая в ветке — строго N/(M-1).
    # Важно: для M>1 НЕ делаем fallback на корень N, иначе может появиться
    # ложный пролёт N -> N/M (пример: 2 -> 2/2), когда контекст ветки потерян.
...
async def find_or_create_acline_segment(
    ...
    if to_connectivity_node_id_if_tap is not None:
        ...
        # Номер отпайки: "<номер_отпайки_в_линии>/<номер_ветки>".
        # Важно: левую часть берём НЕ из номера опоры-якоря, а из номера новой опоры
        # вида N/1 (N — это номер отпайки в линии).
        tap_number_str = None
        tap_root = None
        new_cn = await db.get(ConnectivityNode, to_connectivity_node_id_if_tap)
        if new_cn and new_cn.pole_id is not None:
            new_pole_obj = await db.get(Pole, new_cn.pole_id)
            new_pn = (getattr(new_pole_obj, "pole_number", None) or "").strip()
            if "/" in new_pn:
                left = new_pn.split("/", 1)[0].strip()
                if left:
                    tap_root = left
        # Fallback для старых/нестандартных данных: берём следующий номер отпайки в линии.
        if tap_root is None:
            max_root = 0
            poles_res = await db.execute(
                select(Pole.pole_number).where(Pole.line_id == power_line_id)
            )
            for (pn_val,) in poles_res.all():
                pn_str = (pn_val or "").strip()
                if "/" not in pn_str:
                    continue
                left = pn_str.split("/", 1)[0].strip()
                try:
                    root_num = int(left)
                except (TypeError, ValueError):
                    continue
                if root_num > max_root:
                    max_root = root_num
            tap_root = str(max_root + 1)
        branch_no = tap_branch_index if tap_branch_index is not None else 1
        tap_number_str = f"{tap_root}/{branch_no}"
        result = await db.execute(
...
async def auto_create_span(
    ...
    if "/" in pn:
        try:
            suffix_num = int(pn.split("/", 1)[1].strip())
        except Exception:
            suffix_num = None
    if pn and "/" not in pn:
        use_tap_pole_id = None
        use_tap_branch_index = None
    # Для отпаек вида N/M сначала пытаемся связать строго по номеру ветки.
    # Это защищает от ошибочного пролёта N -> N/2 при потере tap_branch_index.
    previous_pole = None
    if "/" in pn:
        previous_pole = await find_previous_pole_in_tap_branch(db, power_line_id, new_pole)
        if previous_pole is None and suffix_num is not None and suffix_num > 1:
            return None
