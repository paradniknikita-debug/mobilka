...
# PSRType внешние ссылки (профиль импорта); sectionalizer — отдельный код типа ПСР.
DISCONNECTOR_PSR_TYPE_RETRACTABLE = "gm:#_6c49d145-bd34-4f17-89f6-34dfc8698998"
DISCONNECTOR_PSR_TYPE_SECTIONALIZER = "gm:#_8e7a2b63-cf45-5a91-bd10-7f1c8e9d0a12"
...
def normalize_equipment_type(equipment_type: Optional[str]) -> str:
    ...
    if "реклозер" in value or "recloser" in value:
        return "recloser"
    if "разрядник" in value or "опн" in value or "arrester" in value or "surge" in value:
...
def map_equipment_type_to_cim_profile(equipment_type: Optional[str]) -> EquipmentProfile:
    ...
    if normalized == "surge_arrester":
        return EquipmentProfile(
            cim_class="SurgeArrester",
            normalized_type=normalized,
            terminal_count=1,
            psr_type_name="SurgeArrester",
            control_area_name="LineControlArea",
        )
    return EquipmentProfile(
...
def disconnector_psr_type_external_ref(psr_subtype: Optional[str]) -> Dict[str, Any]:
    """
    Внешняя ссылка PowerSystemResource.PSRType для разъединителя на ЛЭП.
    psr_subtype: sectionalizer | отделитель -> отдельный тип; иначе выкатной/по умолчанию.
    """
    v = (psr_subtype or "").strip().lower()
    if v in ("sectionalizer", "section", "отделитель", "отделит"):
        return {"resource": DISCONNECTOR_PSR_TYPE_SECTIONALIZER}
    return {"resource": DISCONNECTOR_PSR_TYPE_RETRACTABLE}
