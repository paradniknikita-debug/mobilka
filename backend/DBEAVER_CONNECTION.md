...
2. **Заполни параметры:**
   ...
   - **Port:** `5433` (или `5432`, если изменил обратно)
   - **Database:** `lepm_db`
   - **Username:** `postgres`
...
### Вариант 2: Пробросить порт (если не проброшен)

Добавь в `docker-compose.yml` в секцию `postgres`:

```yaml
postgres:
  ports:
    - "5432:5432"  # Добавь эту строку
```
